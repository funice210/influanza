# -*- coding: utf-8 -*-
"""
Created on Tue Nov  6 11:20:54 2018
...
# -*- coding: utf-8 -*-
"""
Created on Tue Mar  6 10:59:59 2018
...
all_df = pd.read_csv('E:\\H3N2 samples orignal\\017.txt',sep = '\t' ,encoding = 'utf-8')
#all_df = pd.read_csv('E:\\probes_GSE52428matrix_ID_add0.txt',sep = '\t',encoding = 'utf-8')
cols=['Label','ID','1007_s_at',
'1053_at',
'117_at',
'121_at',
'1255_g_at',
...
while i<101:
    ...
    Train4D_train_Features=train_Features.reshape(train_Features.shape[0],150,150,1).astype('float32')
    Test4D_test_Features=test_Features.reshape(test_Features.shape[0],150,150,1).astype('float32')
    ...
    model.add(Conv2D(filters=16,#建立16個濾鏡filter weight
                     kernel_size=(15,15),#設定每個濾鏡3X3大小
                     padding='same',#此設定讓卷積運算，產生的卷積影像大小不變
                     input_shape=(150,150,1),#第1 2維度:代表輸入的影像形狀150x150大小，第3維度:最後維度是1
                     activation='relu'))#設定ReLU激活函數
    model.add(MaxPooling2D(pool_size=(2,2)))#縮減取樣，將16個150x150影像，縮小為16個75x75影像
    model.add(Dropout(0.5))#訓練迭代時，隨機在神經網路中放棄50%的神經元
    
    model.add(Conv2D(filters=32,#建立16個濾鏡filter weight
                     kernel_size=(3,3),#設定每個濾鏡15X15大小
                     padding='same',#此設定讓卷積運算，產生的卷積影像大小不變
                     input_shape=(75,75,1),#第1 2維度:代表輸入的影像形狀150x150大小，第3維度:最後維度是1
                     activation='relu'))#設定ReLU激活函數
    model.add(MaxPooling2D(pool_size=(3,3)))#縮減取樣，將16個150x150影像，縮小為16個75x75影像
    model.add(Dropout(0.5))#訓練迭代時，隨機在神經網路中放棄50%的神經元
    ...
    all_Features,Label=PreprocessData(all_df)
    all_Features=all_Features.reshape(all_Features.shape[0],150,150,1).astype('float32')
    all_probability=model.predict_classes(all_Features)
    predict=all_probability[:15]
    ...
    #f1 = open('E:\Research\prediction100次結果\H3N2原始資料訓練_BaselinePrechellenge\H3N2_CNN_prediction_100次_origonal_15*15.txt', 'a', encoding = 'UTF-8')
    #f2 = open('E:\Research\prediction100次結果\H3N2_CNN\H3N2_CNN_reality.txt_100次', 'a', encoding = 'UTF-8')    
    #f1 = open('E:\Research\prediction100次結果\H3N2_單人測試_P0\H3N2_CNN_prediction_100次_017單人測試_P0.txt', 'a', encoding = 'UTF-8')
    #f2 = open('E:\Research\prediction100次結果\H3N2_單人測試_P0\H3N2_CNN_prediction_100次_017單人時間_P0.txt', 'a', encoding = 'UTF-8') 
