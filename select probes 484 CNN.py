...
all_df = pd.read_csv('E:\\probes_GSE52428matrix -484_out.txt',sep = '\t',encoding = 'utf-8')
cols=['Label','ID',
'202985_s_at',
'220784_s_at',
'212737_at',
...
Train4D_train_Features=train_Features.reshape(train_Features.shape[0],25,20,1).astype('float32')
Test4D_test_Features=test_Features.reshape(test_Features.shape[0],25,20,1).astype('float32')
...
model.add(Conv2D(filters=16,#建立16個濾鏡filter weight
                 ...
                 input_shape=(25,20,1),#第1 2維度:代表輸入的影像形狀22x22大小，第3維度:最後維度是1
                 activation='relu'))#設定ReLU激活函數
model.add(MaxPooling2D(pool_size=(2,2)))#縮減取樣，將16個22x22影像，縮小為16個11x11影像
...
#建立輸出層
model.add(Dense(300, kernel_initializer='uniform', activation='relu'))#共有30個神經元
