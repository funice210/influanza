...
while i<100:
    ...
    model.add(Conv2D(filters=32,#建立32個濾鏡filter weight
                     kernel_size=(3,3),#設定每個濾鏡3X3大小
                     padding='same',#此設定讓卷積運算，產生的卷積影像大小不變
                     input_shape=(75,75,1),#第1 2維度:代表輸入的影像形狀75x75大小，第3維度:最後維度是1
                     activation='relu'))#設定ReLU激活函數
    model.add(MaxPooling2D(pool_size=(3,3)))#縮減取樣，將32個75x75影像，縮小為32個25x25影像
    model.add(Dropout(0.5))#訓練迭代時，隨機在神經網路中放棄50%的神經元
    
    model.add(Conv2D(filters=64,#建立32個濾鏡filter weight
                     kernel_size=(3,3),#設定每個濾鏡3X3大小
                     padding='same',#此設定讓卷積運算，產生的卷積影像大小不變
                     input_shape=(25,25,1),#第1 2維度:代表輸入的影像形狀25x25大小，第3維度:最後維度是1
                     activation='relu'))#設定ReLU激活函數
    model.add(MaxPooling2D(pool_size=(5,5)))#縮減取樣，將64個25x25影像，縮小為64個5x5影像
    model.add(Dropout(0.5))#訓練迭代時，隨機在神經網路中放棄50%的神經元
    ...
    f1 = open('E:\Research\prediction100次結果\H3N2_CNN使用三層卷積池化\H3N2_CNN_prediction3_100次.txt', 'a', encoding = 'UTF-8')
