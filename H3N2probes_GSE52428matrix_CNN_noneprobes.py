...
all_df = pd.read_csv('E:\\probes_GSE52428matrix.txt',sep = '\t',encoding = 'utf-8')
...
Train4D_train_Feature=train_Features.reshape(train_Features.shape[0],150,150,1).astype('float32')
Test4D_test_Label=test_Features.reshape(test_Features.shape[0],150,150,1).astype('float32')
...
model.add(Conv2D(filters=16,#建立16個濾鏡filter weight
                 ...
                 input_shape=(22277,1,1),#第1 2維度:代表輸入的影像形狀22277X1大小，第3維度:最後維度是1
                 activation='relu'))#設定ReLU激活函數
model.add(MaxPooling2D(pool_size=(2,2)))#縮減取樣，將16個22277x1影像，縮小為16個14x14影像

#建立卷積層2與池化層2
model.add(Conv2D(filters=2,#建立36個濾鏡filter weight
                 kernel_size=(5,5),#設定每個濾鏡5X5大小
                 padding='same',#此設定讓卷積運算，產生的卷積影像大小不變
                 activation='relu'))#設定ReLU激活函數
...
train_history=model.fit(x=Train4D_train_Feature,
                        y=Test4D_test_Label,validation_split=0.2,
